...
class CustomReplayBuffer(ReplayBuffer):
    def add(self, agent_id, experience):
        # Add an experience to the buffer along with the agent's identifier
        super().add(experience, agent_id=agent_id)

    def sample_by_agent(self, agent_id, batch_size):
        # Sample experiences generated by a specific agent
        agent_experiences = [exp for exp in self.buffer if exp.agent_id == agent_id]
        return random.sample(agent_experiences, min(batch_size, len(agent_experiences)))
...
class MultiAgentPolicyManager(BasePolicy):
    ...
    def learn(
        ...
        **kwargs: Any,
        ...
        results = {}
        for agent_id, policy in self.policies.items():
            # Sample experiences for the specific agent
            agent_batch = buffer.sample_by_agent(agent_id, batch_size)
            if not agent_batch.is_empty():
                out = policy.learn(batch=agent_batch, **kwargs)
