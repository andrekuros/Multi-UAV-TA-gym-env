# -*- coding: utf-8 -*-
"""
Created on Fri Mar 31 15:30:24 2023
...
def batchify_obs(observations):
    batched_obs = []

    for obs in observations.values():
        combined_obs = np.concatenate((obs['agent_position'], obs['target_status']))
        batched_obs.append(combined_obs)

    numpy_array = np.stack(batched_obs, axis=0)
    tensor_input = torch.from_numpy(numpy_array).float().to(device)

    # Redimensione o tensor de entrada para atender às expectativas do conv2d()
    tensor_input = tensor_input.view(-1, 4, 64, 64)  # Altere 4 para o número de canais da sua observação, e 64x64 para o tamanho da imagem
...
if __name__ == "__main__":
    ...
    env = mdenv.env(action_mode= "TaskAssign", render_speed=render_speed, num_drones=num_drones, num_targets=num_targets, max_time_steps=max_cycles)
    ...
    num_actions = num_drones #env.action_space[first_agent].n
